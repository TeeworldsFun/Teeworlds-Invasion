...
void CCharacterCore::Tick(bool UseInput)
{
	...
	float MaxSpeed = 0.f;        // Grounded ? m_pWorld->m_Tuning.m_GroundControlSpeed : m_pWorld->m_Tuning.m_AirControlSpeed;
	float Accel = 0.f;	       // Grounded ? m_pWorld->m_Tuning.m_GroundControlAccel : m_pWorld->m_Tuning.m_AirControlAccel;
	float Friction = 0.f;        // Grounded ? m_pWorld->m_Tuning.m_GroundFriction : m_pWorld->m_Tuning.m_AirFriction;
	...
	if (UseInput)
	{
		...
		else
		{
			m_HookedPlayer = -1;
			m_HookState = HOOK_GRABBED;
			m_HookPos = vec2(m_Pos.x + m_Input.m_TargetX * 2, 1000.0f * 32.f);
		}
	}
	...
	else if (m_HookState >= HOOK_GRABBED && m_HookState < HOOK_GRABBED)
	{
		m_HookState++;
	}
...
{
	float RampValue = 0; // VelocityRamp(length(m_Vel) * 50, m_pWorld->m_Tuning.m_VelrampStart, m_pWorld->m_Tuning.m_VelrampRange, m_pWorld->m_Tuning.m_VelrampCurvature);
